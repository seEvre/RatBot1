...
async def get_or_create_backup_logs_channel(guild):
...
async def post_backup_log(guild, filename):
    log_channel = await get_or_create_backup_logs_channel(guild)
    url = f"{os.getenv('RENDER_EXTERNAL_URL')}/backups/{filename}"
    embed = Embed(
        title="Backup Created",
        description=f"Backup created at <t:{int(datetime.utcnow().timestamp())}:f>"
    )
...
async def auditspam(interaction: Interaction):
    ...
    for i in range(50):
        ...
        try:
            ...
            # zero delay for audit spam per request
...
async def on_ready():
    ...
    # Register slash commands fresh on startup
    await bot.sync_commands(guild_id=GUILD_ID)

    global backup_task
    if backup_task is None or backup_task.done():
        backup_task = bot.loop.create_task(auto_backup_loop())
